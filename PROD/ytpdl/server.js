...
httpServer.listen(8002, () => console.log("Serveur en écoute sur le port 8002"));
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", function () {
    ...
    if (fs.existsSync(__dirname + "/private/" + socket.id + ".zip"))
      fs.unlinkSync(__dirname + "/private/" + socket.id + ".zip");
    if (fs.existsSync(__dirname + "/private/" + socket.id))
      fs.rmdir(__dirname + "/private/" + socket.id, { recursive: true }, () => console.log("DELETE OK"));
  });
  ...
  app.get('/telecharger', async (req, res) => {
    try {
      ...
      res.download(filename, "arthurdev.zip", (err) => {
        if (err)
          console.log(err)
        if (fs.existsSync(__dirname + "/private/" + req.query.id)) {
          fs.rmdir(__dirname + "/private/" + req.query.id, { recursive: true, }, () => console.log("DELETE OK"));
          fs.unlinkSync(__dirname + "/private/" + req.query.id + ".zip");
        }
        socket.ready = true;
      });
    } catch (err) {
...
const telechargerFichier = (id, title, socket) => {
  if (!fs.existsSync(__dirname + "/private/" + socket.id))
    fs.mkdirSync(__dirname + "/private/" + socket.id);
  downloader_audio.outputPath = __dirname + "/private/" + socket.id;

  // Télécharge la vidéo.F
  downloader_audio.download(id);

  // Télécharge le thumbnail.
  let image_options = {
    url: "https://img.youtube.com/vi/" + id + "/maxresdefault.jpg",
    dest: __dirname + "/private/" + socket.id + "/" + title + ".jpg",
  };
  downloader_image.image(image_options).then(({ filename }) => console.log("Image sauvegardée : ", filename))
    .catch(() => {
      image_options = {
  ...
  downloader_audio.on("queueSize", (size) => {
    ...
    if (size == 0 && socket.ready) {
      ...
      let folder = __dirname + "/private/" + socket.id;

      zipDossier(folder, folder + ".zip").then(() => socket.emit("zip-rd"));
    }
  });

  // Pourcentage de progression.
  downloader_audio.on("progress", (progress) => socket.emit("dl-update", Math.round(JSON.parse(JSON.stringify(progress)).progress.percentage) + "%"));
